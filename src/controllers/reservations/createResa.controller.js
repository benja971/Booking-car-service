...
async function createResa(req, res) {
	...
	if (startDate < Date.now() || endDate < Date.now()) return res.status(400).send({ message: "Dates must be after today" });

	// check if start is before end
	if (new Date(startDate) >= new Date(endDate)) return res.status(400).send({ message: "Start date must be before end date" });
	...
	if (isOverlapingResa) return res.status(400).send({ message: "This car is already reserved for this period" });
